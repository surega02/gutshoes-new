...
function Checkout({ cart, navigate, createOrder, user, addresses }) {
  const [step, setStep] = useState(1); const [shipping, setShipping] = useState(18000); const [voucher, setVoucher] = useState(''); const [voucherValue, setVoucherValue] = useState(0); const [error, setError] = useState(''); const [submitting,setSubmitting]=useState(false);
  const next = e => { e.preventDefault(); const data = new FormData(e.currentTarget); if (step === 1 && (!data.get('name') || !data.get('email') || !data.get('phone'))) {setError('Lengkapi nama, email, dan nomor telepon untuk melanjutkan.');return;} setError(''); setStep(Math.min(3, step+1)); };
  const applyVoucher = () => { if(voucher.toUpperCase()==='DEMO20'){setVoucherValue(20000);setError('')}else{setVoucherValue(0);setError('Kode voucher tidak valid. Coba DEMO20 untuk simulasi.')}};
  const submitOrder=()=>{if(submitting)return;setSubmitting(true);window.setTimeout(()=>{createOrder();navigate('payment')},350)};
...
function LoginV2({ navigate, onLogin, returnTo }) {
  ...
  return <main className="auth-login"><section className="login-copy"><button className="back-link" onClick={() => navigate('home')}><Icon name="back"/> Kembali ke toko</button><h1>Satu akun untuk langkah berikutnya.</h1><p>Simpan alamat, lihat riwayat pesanan, dan lanjutkan belanja dengan data yang sudah dikenali.</p><div className="auth-route" aria-hidden="true"><span>Temukan</span><i/><span>Beli</span><i/><strong>Ikuti</strong></div></section><section className="login-panel" aria-labelledby="login-title"><img src={logoGutShoes} alt=""/><h2 id="login-title">Masuk ke GutShoes</h2><p>Gunakan akun Google sebagai identitas utama. Email dari Google tidak dapat diubah di GutShoes.</p><Feedback type="success">Simulasi frontend—belum terhubung ke Google OAuth.</Feedback><Feedback>{status === 'error' ? 'Koneksi simulasi gagal. Periksa jaringan lalu coba masuk kembali.' : ''}</Feedback><Button className="google-button" onClick={login} disabled={status === 'loading'} aria-busy={status === 'loading'}><span className="google-mark" aria-hidden="true">G</span>{status === 'loading' ? 'Menghubungkan akun…' : 'Lanjutkan dengan Google'}</Button><button className="text-link login-error-demo" onClick={() => setStatus('error')}>Uji kondisi koneksi gagal</button><small>Guest checkout tetap tersedia tanpa membuat akun.</small></section></main>;
}
...
export default function App() {
  ...
  const [user,setUser]=useState(()=>{try{return JSON.parse(localStorage.getItem('gutshoes-demo-user'))}catch{return null}});
  const [addresses,setAddresses]=useState(()=>{try{return JSON.parse(localStorage.getItem('gutshoes-demo-addresses'))||[]}catch{return []}});
  ...
  useEffect(()=>{if(user)localStorage.setItem('gutshoes-demo-user',JSON.stringify(user));else localStorage.removeItem('gutshoes-demo-user')},[user]);
  useEffect(()=>localStorage.setItem('gutshoes-demo-addresses',JSON.stringify(addresses)),[addresses]);
  ...
  const addToCart=(product,size)=>setCart(prev=>{const ix=prev.findIndex(i=>i.product.id===product.id&&i.size===size);if(ix>=0)return prev.map((i,n)=>n===ix?{...i,qty:Math.min(i.qty+1,product.stock[size])}:i);return [...prev,{product,size,qty:1}]});
  ...
  const createOrder=()=>{const subtotal=cart.reduce((s,i)=>s+i.product.price*i.qty,0);setOrder({number:'GS-20260822-00001',total:subtotal+18000});};
  const login=()=>setUser({name:'Ega Pratama',email:'ega@example.com',phone:'081234567890',created:'22 Agustus 2026, 15.42 WIB',updated:'22 Agustus 2026, 15.42 WIB'});
  const logout=()=>{setUser(null);setAddresses([]);setReturnTo('profile');navigate('home')};
  ...
  if(page==='checkout')content=<Checkout cart={cart} navigate={navigate} createOrder={createOrder} user={user} addresses={addresses}/>;
